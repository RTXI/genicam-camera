...
QArvMainWindow::QArvMainWindow(QWidget *parent, bool standalone_)
  ...
  /*
    ...
    if (usingFallbackIcons)
      logMessage() << "Some icons are not available in your theme, using bundled
    icons.";
  */
  {
  ...
  if (camera != NULL) {
    ...
    if (start && !started) {
      ...
      if (decoder != NULL) {
        ...
        started = true;
        foreach (auto wgt, toDisableWhenPlaying) { wgt->setEnabled(false); }
    ...
    if (failures.count() != 0) {
      QString message = "<html><head/><body><p>" +
                        tr("Settings could not be completely loaded. "
                           "This can happen because camera features are "
                           "interdependent and may "
                           "require a specific loading order. The following "
                           "settings failed:") +
                        "</p>";
...
void QArvMainWindow::on_closeFileAction_triggered(bool checked) {
  ...
  foreach (auto wgt, toDisableWhenPlaying) { wgt->setEnabled(!started); }
...
void QArvMainWindow::histogramNextFrame() { drawHistogram = true; }
