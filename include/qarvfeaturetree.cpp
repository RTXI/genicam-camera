...
void QArvCamera::QArvFeatureTree::recursiveSerialization(
	...
	if (ARV_IS_GC_REGISTER_NODE(node) && 
		...
		out << "Enumeration\t" << 
			arv_gc_enumeration_get_string_value(ARV_GC_ENUMERATION(node), NULL) << endl;
	} else if (ARV_IS_GC_STRING(node)) {
		out << "String\t" << arv_gc_string_get_value(ARV_GC_STRING(node), NULL) << endl;
		...
		out << "Boolean\t" << arv_gc_boolean_get_value(ARV_GC_BOOLEAN(node), NULL) << endl;
	} else if (ARV_IS_GC_INTEGER(node)) {
		out << "Integer\t" << arv_gc_integer_get_value(ARV_GC_INTEGER(node), NULL) << endl;
	}
}
